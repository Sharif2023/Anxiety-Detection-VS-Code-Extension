...
type DayKey = string; // e.g. "2025-08-29"
type SessionEvent = { type: "idle" | "resume"; at: number; durationMs?: number };
...
const DAY_KEY = (): DayKey => new Date().toISOString().slice(0, 10);

function getStats(context: vscode.ExtensionContext): DayStats {
  const day = DAY_KEY();
  const existing = context.globalState.get<DayStats>(day);
...
function setStats(context: vscode.ExtensionContext, stats: DayStats) {
  const day = DAY_KEY();
...
function fileKeyFromUri(uri: vscode.Uri | undefined): string | undefined {
...
function ensureFileEntry(stats: DayStats, key: string) {
  if (!stats.perFile[key]) {
    stats.perFile[key] = { keystrokes: 0, activeMs: 0 };
  }
}

async function saveFileSuggesting(
...
function buildCsvString(s: DayStats, includePerFile: boolean, extras: {
  undoCount: number; redoCount: number; compileAttempts: number; jsErrors: number;
}) {
  const header = [
...
async function uploadCsvIfEnabled(context: vscode.ExtensionContext, reason: string, extras: {
  undoCount: number; redoCount: number; compileAttempts: number; jsErrors: number;
}) {
  const cfg = vscode.workspace.getConfiguration("activityTracker");
  const enabled = cfg.get<boolean>("autoUpload.enabled", false);
  if (!enabled) return;

  const endpoint = (cfg.get<string>("autoUpload.endpoint", "") || "").trim();
  if (!endpoint) {
    vscode.window.showWarningMessage("Activity Tracker: autoUpload enabled but no endpoint configured.");
    return;
  }
  const token = (cfg.get<string>("autoUpload.authToken", "") || "").trim();
  const includePerFile = cfg.get<boolean>("autoUpload.includePerFile", true);
...
export function activate(context: vscode.ExtensionContext) {
  const idleMsCfg = () =>
    vscode.workspace.getConfiguration("activityTracker").get<number>("idleMs", 60000);
  ...
  let currentFileKey: string | undefined = fileKeyFromUri(vscode.window.activeTextEditor?.document.uri);
  ...
  const d4 = vscode.window.onDidChangeWindowState((state) => {
    if (!logging) { updateStatus(); return; }
    if (state.focused) {
      stats = getStats(context);
  ...
  const showStats = vscode.commands.registerCommand("activityTracker.showStats", () => {
    const panel = vscode.window.createWebviewPanel(
      "activityTracker.live",
      "Activity Tracker — Live",
...
<body>
  ...
  <script>
    ...
    function buildPayload() {
      ...
      entries.sort((a, b) => {
        const av = a[1], bv = b[1];
        if (bv.activeMs !== av.activeMs) return bv.activeMs - av.activeMs;
      ...
      const idleRowsHtml = s.idleEvents.map((ev) => {
        ...
        if (ev.type === "idle") return '<tr><td>Idle</td><td>' + time + '</td><td>…</td></tr>';
  ...
  const resetToday = vscode.commands.registerCommand("activityTracker.resetToday", async () => {
    const ok = await vscode.window.showWarningMessage(
      "Reset today's Activity Tracker stats?",
      { modal: true },
      "Reset"
    );
    if (ok === "Reset") {
  ...
  const exportTodayJSON = vscode.commands.registerCommand("activityTracker.exportTodayJSON", async () => {
    ...
    const payload = {
      ...
      perFile: Object.fromEntries(
        Object.entries(s.perFile).map(([k, v]) => [k, { keystrokes: v.keystrokes, activeMs: v.activeMs }])
      )
    };
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("activityTracker.trackUndo", async () => {
      undoCount++;
  ...
  const cfg = vscode.workspace.getConfiguration("activityTracker");
  let uploadIntervalMin = cfg.get<number>("autoUpload.intervalMinutes", 30);
  ...
  function startAutoUploadTimer() {
    ...
    uploadIntervalMin = vscode.workspace.getConfiguration("activityTracker").get<number>("autoUpload.intervalMinutes", 30);
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeConfiguration(e => {
      if (
        e.affectsConfiguration("activityTracker.autoUpload.intervalMinutes") ||
  ...
  context.subscriptions.push(
    vscode.window.onDidChangeWindowState(s => {
      if (s.focused) uploadCsvIfEnabled(context, "focus", { undoCount, redoCount, compileAttempts, jsErrors });
    })
  );
  ...
  context.subscriptions.push(
    ...
    { dispose: () => { if (autoUploadTimer) clearInterval(autoUploadTimer); } }
  );
